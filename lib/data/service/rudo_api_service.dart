...
class RudoApiService{
  ...
  Future <List<Album>> getAlbums() async{
    var res = await Request.instance.get("albums");
    if(res.statusCode == 200){
       return (res.data as List).map((e) => Album.fromJson(e)).toList();
    }else{
      throw Exception("Error en apiService getAlbums. StatusCode: ${res.statusCode} Data: ${res.data}");
    }
  }

   Future<List<Photo>> getAlbumPhotosById(String id)async{
    var res = await Request.instance.get("albums/$id");
    if(res.statusCode == 200){
       return (res.data['photos'] as List).map((e) => Photo.fromJson(e)).toList();
    }else{
      throw Exception("Error en apiService getAlbums. StatusCode: ${res.statusCode} Data: ${res.data}");
    }
   }
